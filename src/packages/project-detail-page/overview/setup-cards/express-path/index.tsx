...
export const ExpressPath = () => {
  ...
  const ok = useMemo(() => expressRepo?.dirExists === true, [expressRepo?.dirExists]);

  if (!expressRepo || expressRepo.localDir === undefined) {
    return null;
  }
  ...
  return (
    <div className={styles.card}>
      ...
      <span className={ok ? styles.labelOk : styles.labelPending}>Express path</span>
      <p className={styles.description}>
        The Express repo path from hub.local.json exists on this machine.
      </p>
    </div>
  );
};
