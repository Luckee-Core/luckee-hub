...
export const ExpressInstalled = () => {
  ...
  const ok = useMemo(
    () => expressRepo?.depsInstalled === true,
    [expressRepo?.depsInstalled],
  );

  if (!expressRepo || expressRepo.dirExists !== true) {
    return null;
  }
  ...
  return (
    <div className={styles.card}>
      ...
      <span className={ok ? styles.labelOk : styles.labelPending}>Express installed</span>
      <p className={styles.description}>
        Dependencies are installed in the Express repo (node_modules is present).
      </p>
    </div>
  );
};
