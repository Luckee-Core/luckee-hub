...
export const ApiRunning = () => {
  ...
  const ok = useMemo(
    () => API_RUNNING_STATUSES.includes(hookStatus),
    [hookStatus],
  );

  if (!expressRepo || expressRepo.depsInstalled !== true) {
    return null;
  }
  ...
  return (
    <div className={styles.card}>
      ...
      <span className={ok ? styles.labelOk : styles.labelPending}>API running</span>
      <p className={styles.description}>
        The Express dev server is listening and the health endpoint responds.
      </p>
    </div>
  );
};
