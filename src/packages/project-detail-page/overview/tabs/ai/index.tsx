...
import { saveAiEnvThunk } from '@/store/thunks/projects';
import { ConfiguredCheck } from '../configured-check';
import { AnthropicApiKeyInput } from './inputs';

const GROUP_ID = 'ai';

/**
 * AI tab — probed on project open after Supabase; read-only when configured.
 */
export const AiTab = () => {
  ...
  const showForm = !configured || projectsBuilder.aiEnvEditing;
  ...
  const statusLabel = useMemo(() => {
    ...
    return probe.keysPresent?.ANTHROPIC_API_KEY === false
      ? 'Missing: ANTHROPIC_API_KEY'
      : 'Not configured';
  }, [probe, configured, projectsBuilder.expressEnvLoadStatus]);

  const canSave = useMemo(
    () => projectsBuilder.anthropicApiKey.trim().length > 0 && !isBusy,
    [projectsBuilder.anthropicApiKey, isBusy],
  );
      ...
      <div className={styles.root}>
        <div className={styles.statusRow}>
          ...
          <button
            ...
            onClick={() => dispatch(ProjectsBuilderActions.setAiEnvEditing(true))}
          >
            Edit
          </button>
        </div>
        ...
        <ConfiguredCheck
          ...
          ok={probe?.keysPresent?.ANTHROPIC_API_KEY === true}
          detail="ANTHROPIC_API_KEY is set in Express .env"
        />
      </div>
    );
  }
      ...
      <div className={styles.statusRow}>
        ...
        {configured ? (
          <button
            ...
            onClick={() => dispatch(ProjectsBuilderActions.setAiEnvEditing(false))}
          >
            Cancel
          </button>
        ) : null}
      ...
      {!probe && isBusy ? (
        ...
        <>
          <p className={styles.hint}>
            Lead Studio uses Claude Haiku in code — only the Anthropic API key is needed here.
          </p>
